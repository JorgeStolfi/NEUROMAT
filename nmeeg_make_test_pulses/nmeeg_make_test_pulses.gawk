#! /usr/bin/gawk -f
# Last edited on 2021-08-30 18:26:52 by stolfi
    ...
    if (type == 0)
      {        
        ...
        for (ip = 0; ip < np_imp; ip++)
          { it_imp[ip] = zt + ip*dt; # Frame of impulse.
            ne++;
          }
      ...
      { 
        ...
        ht_rect = int(0.05*nt); # Half-width of rectangular pulse (frames).
        ne++;
        ...
        for (ip = 0; ip < np_sine; ip++) 
          { fr = (60*(ip + 0.5))/np_sine;
            fr_sine[ip] = fr; # Frequency of sinusoid (Hz). 
            am_sine[ip] = 20; # Amplitude of sinusoid.
            ne++;
          }
      }
    else if (type == 2)
