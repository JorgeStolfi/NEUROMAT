...
/* Last edited on 2017-07-24 21:36:59 by stolfilocal */
...
typedef struct nmsim_cohort_mf_state_t
  { double V_avg; /* Mean potential. */
    ...
    double M;     /* Leakage modulator. */
    double G;     /* Input gain modulator. */
    double H;     /* Output gain modulator. */
  ...
  /* Stochastic state of a /cohort/, a set of neurons with the 
    ...
    the same firing modulator {G}, and same output modulator {H}.
...
void nmsim_cohort_mf_state_set
  ...
  /* Sets the state parameters of a cohort {cs} to potential {V_avg ±
    V_dev}, leakage modulator {M}, input modulator {G}, output modulator {H}, and relative
    fraction {eta}.
