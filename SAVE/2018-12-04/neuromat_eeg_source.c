...
/* Last edited on 2013-06-05 21:48:24 by stolfilocal */
...
void neuromat_eeg_source_write(FILE *wr, char *pref, neuromat_eeg_source_t *ho)
  {
    ...
    neuromat_eeg_header_write_field_int(wr, pref, "nt", ho->nt, 1, INT_MAX);
    neuromat_eeg_header_write_field_int_range(wr, pref, "sample_range", ho->it_ini, ho->it_fin, 0, INT_MAX);
    neuromat_eeg_header_write_field_double(wr, pref, "fsmp", ho->fsmp, 0.01, 1.0e12);
    neuromat_eeg_header_write_field_int(wr, pref, "subject", ho->subject, 1, INT_MAX);
    neuromat_eeg_header_write_field_int(wr, pref, "run", ho->run, 1, INT_MAX);
    fflush(wr);
  }
...
void neuromat_eeg_source_read_field(FILE *rd, char *name, neuromat_eeg_source_t *ho)
  {
    if (strcmp(name, "file") == 0) 
      ...
      { ho->it_ini = fget_int(rd); 
        ...
        demand((ho->it_ini >= 0) && (ho->it_fin >= ho->it_ini), "invalid sample index range");
      }
