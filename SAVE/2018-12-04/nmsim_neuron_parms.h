...
/* Last edited on 2017-07-24 22:36:53 by stolfilocal */
...
typedef struct nmsim_neuron_parms_t
  ...
  /* Parameters of a GL neuron.
    ...
    {X[i,t]=1}), the state {(V[i,t+1],c[i,t+1],G[i,t+1],H[i,t+1])} is reset to
    {(V_R,c_B,G_R,H_R)}.

    If the neuron does not fire between times {t} and {t+1} (that is, if
    {X[i,t]=0}), the state at {t+1} is

      { V[i,t+1] = V_B + c[i,t]*(V[i,t] - V_B) + I[i,t] + \sum_{j} (X[j,t]*H[j,t]*w[j-->i]) }

      { c[i,t+1] = c_B + c_mu*(c[i,t] - c_B) }

      { G[i,t+1] = 1 - G_mu*(1 - G[i,t]) }

      { H[i,t+1] = 1 - H_mu*(1 - H[i,t]) }
...
#define nmsim_neuron_parms_read_INFO \
  ...
  " factors {V_mu}, {G_mu}, and {H_mu} using the specified" \
