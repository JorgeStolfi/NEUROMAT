...
/* NeuroMat tools. */
/* Last edited on 2023-10-22 02:55:21 by stolfi */
...
void neuromat_eeg_get_channel_names(int ne, int nv, char **evname, int *ncP, char ***chnameP);
  /* Returns in {*chnameP} a vector {chname[0..nc-1]} of {nc} strings with 
...
void neuromat_eeg_get_20_channel_names(int *ncP, char ***chnameP);
  /* Returns in {*chnameP} a vector {chname[0..20]} of 21 strings that
...
void neuromat_eeg_get_128_channel_names(int *ncP, char ***chnameP);
  /* Returns in {*chnameP} a vector {chname[0..128]} of 129 strings with 
    the names of the channels for the 128-electrode EEG datasets. 
    The number of channels (129) is returned in {*ncP}.
    
    Specifically, names {chname[0..127]} are "C1" to "C128" 
    (ie. {chname[i]} is "C{i+1}"), for {i} in {0..127}.  
    The last channel is assumed to be the reference (ground) electrode,
    so {chname[128]} is set to "CZ" (name used by the Neuromat team; 
